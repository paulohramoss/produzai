...
const mobileNav = [...nav, { id: 'settings' as Page, label: 'Config.', icon: <Settings size={18} /> }]
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-surface-card border-t border-surface-border flex">
        {mobileNav.slice(0, 5).map((item) => (
          <button
            key={item.id}
            onClick={() => setPage(item.id)}
            className={`flex-1 flex flex-col items-center justify-center py-2 gap-0.5 text-[10px] font-medium transition-colors ${
              currentPage === item.id ? 'text-brand-400' : 'text-gray-500'
            }`}
          >
            {item.icon}
            {item.label}
